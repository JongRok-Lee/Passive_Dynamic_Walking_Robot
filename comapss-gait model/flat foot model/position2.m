...
if rem(n0,2) == 1
    % leg1의 발 끝
    p_1t0 = repmat(xi0,length(t),1);
    % leg1의 다리 중심
    p_01b =p_1t0 - d*[cos(x(:,1)) sin(x(:,1))];
    % leg1의 뒤꿈치
    p_01hb = p_1t0 - (c+d)*[cos(x(:,1)) sin(x(:,1))];
    % 골반의 위치
    ...
    % leg2의 발 끝
    p_2t0 = repmat(xi0,length(t),1);
    % leg2의 다리 중심
    p_02b =p_2t0 - d*[cos(x(:,1)) sin(x(:,1))];
    % leg2의 뒤꿈치
    p_02hb = p_2t0 - (c+d)*[cos(x(:,1)) sin(x(:,1))];
    % 골반의 위치
